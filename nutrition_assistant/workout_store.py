...
Stores data in routines.json, workouts.json, and prs.json in the same directory.
"""
...
def _save_workouts(data: dict):
    _save_json(WORKOUTS_FILE, data)
...
def update_routine(routine_id: str, data: dict) -> dict:
...
def start_workout(
    ...
    if routine_id and routine_day_id:
        ...
        for ex_def in day.get("exercises", []):
            ...
            prev_sets = get_previous_sets(exercise_id)
...
def add_exercise_to_workout(workout_id: str, exercise_id: str) -> dict:
...
def remove_exercise_from_workout(workout_id: str, exercise_id: str):
...
def update_set(workout_id: str, exercise_id: str, set_id: str, data: dict) -> dict:
...
def add_set(workout_id: str, exercise_id: str) -> dict:
...
def delete_set(workout_id: str, exercise_id: str, set_id: str):
...
def finish_workout(workout_id: str, notes: str = "") -> dict:
...
def discard_workout(workout_id: str):
...
def list_workouts(limit: int = 20, offset: int = 0) -> list:
...
def get_previous_sets(exercise_id: str) -> list:
