...
from calculator import (
    calculate_bmr, calculate_tdee, ACTIVITY_LEVELS, GOAL_ADJUSTMENTS,
    PROTEIN_FACTORS, FAT_FACTORS, NEAT_FACTORS,
)
...
def adjust_macros_for_training(profile: dict, weekly_summary: dict) -> dict:
    ...
    protein_g = weight * PROTEIN_FACTORS.get(goal, 2.0)
    fat_g     = weight * FAT_FACTORS.get(goal, 0.8)
...
def print_macro_report(adj: dict, summary: dict) -> None:
    ...
    print("─"*58)
    pf = PROTEIN_FACTORS.get(adj.get("goal", "maintain"), 2.0)
    print(f"  Proteínas ({pf}g/kg):  {adj['protein_g']:>5} g  →  {adj['protein_g']*4:>5} kcal")
